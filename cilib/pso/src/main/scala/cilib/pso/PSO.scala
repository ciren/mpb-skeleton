...
object PSO {
  ...
  def quantum(
    ...
    Step.liftR {
      for {
        r_i <- x.traverse(_ => Dist.stdNormal) // N(0.0, 1.0))
        originSum = math.sqrt(r_i.pos.foldLeft(0.0)((a, c) => a + c * c))
        scale <- r.flatMap(a => dist(0.0, a)) // Use the provided distribution to scale the cloud radius
      } yield (scale / originSum) *: r_i
    }
