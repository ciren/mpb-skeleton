...
object FileOutput extends SafeApp {
  ...
  val absolute = Environment(
    ...
    eval = Eval.unconstrained((x: NonEmptyList[Double]) => Feasible(Benchmarks.absoluteValue(x)))
  )

  val ackley = Environment(
    cmp = Comparison.dominance(Max),
    eval = Eval.unconstrained((x: NonEmptyList[Double]) => Feasible(Benchmarks.ackley(x)))
  )
  val quadric = Environment(
    cmp = Comparison.dominance(Max),
    eval = Eval.unconstrained((x: NonEmptyList[Double]) => Feasible(Benchmarks.quadric(x)))
  )
  ...
  // TODO: Cleanup - remove Environment defines above FIXME
  ...
  def performanceMeasure(
      eval: Problem[Unit, Double],
      collection: (Unit, NonEmptyList[Entity[Mem[Double], Double]])
  ) = {
    val feasibleOptic = Lenses._singleFitness[Double].composePrism(Lenses._feasible)
    val fitnessValues =
      collection._2.map(x => feasibleOptic.headOption(x.pos).getOrElse(Double.PositiveInfinity))
  ...
  def simulation(env: Environment[Double], stream: Process[Task, Problem[Unit, Double]]) =
    List(Runner.staticAlgorithm("GBestPSO", gbestIter),
         Runner.staticAlgorithm("LBestPSO", lbestIter))
      .map(alg => Runner.foldStep(env.cmp, rng, swarm, alg, stream, onChange))
  ...
  def writeResults(choice: Choice): Process[Task, Unit] = {
    val measured: Process[Task, Process[Task, Measurement[Results]]] =
      Process.emitAll(simulations.map(_.take(1000).pipe(Runner.measure(performanceMeasure))))
