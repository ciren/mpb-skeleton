...
object FitnessTest extends Properties("Fitness") {
  ...
  def simpleViolationGen: Gen[Constraint[Int]] =
    for {
      ...
      function = new ConstraintFunction[Int] { def apply(x: NonEmptyList[Int]) = 0.0 }
      constraint <- Gen.oneOf(
        LessThan(value, function),
        LessThanEqual(value, function),
        Equal(value, function),
        GreaterThan(value, function),
        GreaterThanEqual(value, function),
        InInterval(spire.math.Interval(-5.12, 5.12), function)
      )
    } yield constraint
