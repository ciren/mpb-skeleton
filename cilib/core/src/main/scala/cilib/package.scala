...
package object cilib extends EvalInstances {
  ...
  implicit class IntervalOps[A](val interval: Interval[A]) extends AnyVal {
    ...
    // def lowerValue: A = lowerBound(interval)
    // def upperValue: A = getValue(interval.upperBound)

    // def range(implicit A: Ring[A]): A =
    //   A.minus(upperValue, lowerValue)

    // def center(implicit A: Field[A]): A =
    //   A.times(A.plus(upperValue, lowerValue), A.fromDouble(0.5))
  }
  ...
  def lowerValue[A: NumberTag](i: Interval[A])(implicit N: NumberTag[A]): A =
    i.lowerBound match {
      case Closed(x) => x
      case Open(x)   => x
      case _         => N.hasMinValue.getOrElse(sys.error("???"))
    }

  def upperValue[A: NumberTag](i: Interval[A])(implicit N: NumberTag[A]): A =
    i.upperBound match {
      case Closed(x) => x
      case Open(x)   => x
      case _         => N.hasMaxValue.getOrElse(sys.error("???"))
    }

  def range[A](i: Interval[A])(implicit A: Ring[A], N: NumberTag[A]): A =
    A.minus(upperValue(i), lowerValue(i))

  def center[A](i: Interval[A])(implicit A: Field[A], N: NumberTag[A]): A =
    A.times(A.plus(upperValue(i), lowerValue(i)), A.fromDouble(0.5))
}
