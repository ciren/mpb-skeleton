...
object Boundary {
  ...
  def absorb[A](implicit N: spire.math.Numeric[A]): Enforce[Need, A] =
    Enforce((a: A, b: Interval[Double]) =>
      Need {
        ...
        if (z < lowerValue(b)) N.fromDouble(lowerValue(b))
        else if (z > upperValue(b)) N.fromDouble(upperValue(b))
        else a
    })
  ...
  def toroidal[A](implicit N: spire.math.Numeric[A]): Enforce[Need, A] =
    Enforce((a: A, b: Interval[Double]) =>
      Need {
        val z = N.toDouble(a)
        val _range = math.abs(range(b))

        if (z < lowerValue(b)) N.fromDouble(upperValue(b) - (lowerValue(b) - z) % _range)
        else if (z > upperValue(b)) N.fromDouble(lowerValue(b) + (z - upperValue(b)) % _range)
        else a
    })
  ...
  def midpoint[A](implicit N: spire.math.Numeric[A]): Enforce[Need, A] =
    Enforce((a: A, b: Interval[Double]) =>
      Need {
        ...
        else N.fromDouble((upperValue(b) + lowerValue(b)) / 2.0)
    })
  ...
  def evolutionary[A](implicit N: spire.math.Numeric[A]): EnforceTo[RVar, A] =
    EnforceTo((a: A, b: Interval[Double], target: A) => {
      ...
      if (z < lowerValue(b))
        Dist.stdUniform.map(x => N.fromDouble(x * lowerValue(b) + (1.0 - x) * N.toDouble(target)))
      else if (z > upperValue(b))
        Dist.stdUniform.map(x => N.fromDouble(x * upperValue(b) + (1.0 - x) * N.toDouble(target)))
      else RVar.pure(a)
    })
