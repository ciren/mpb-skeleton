...
//      )
  ...
  scmInfo := Some(
    ScmInfo(url("https://github.com/cirg-up/cilib"), "scm:git:git@github.com:cirg-up/cilib.git")),
  ...
  previousArtifactVersion := { // Can this be done nicer/safer?
    ...
    name.getOrElse("NONE").replace("refs/tags/v", "")
  },
...
lazy val cilib = project
  ...
  .settings(cilibSettings ++ credentialSettings ++ noPublishSettings ++ Seq(
...
lazy val example = project
  ...
  .settings(
    cilibSettings ++ noPublishSettings ++ Seq(
      //test in assembly := {},
      packMain := Map("main" -> "cilib.example.ProfPSO"),
      packJvmOpts := Map("main" -> Seq("-Xmx2048m")),
      // mainClass in assembly := Some("cilib.example.ProfPSO"),
      // assemblyJarName in assembly := "pso-gbest.jar",
      fork in run := true,
